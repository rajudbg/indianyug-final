...
const generateMainSitemap = async () => {
  console.log('🔄 Fetching posts for main sitemap...');
  const posts = await fetchPosts();
  
  if (posts.length === 0) {
    console.warn('⚠️ No posts found for main sitemap');
    return;
  }

  console.log(`📄 Found ${posts.length} posts for main sitemap`);
...
${posts.map(post => `  <url>
  ...
  console.log(`📊 Main sitemap contains ${posts.length} posts with proper dates`);
};

// Generate news sitemap
const generateNewsSitemap = async () => {
  console.log('🔄 Fetching posts for news sitemap...');
  const posts = await fetchPosts();
  
  if (posts.length === 0) {
    console.warn('⚠️ No posts found for news sitemap');
    return;
  }

  console.log(`📰 Found ${posts.length} posts for news sitemap`);
...
${posts.map(post => `  <url>
  ...
  console.log(`📊 News sitemap contains ${posts.length} news articles`);
};

// Run both sitemap generations
